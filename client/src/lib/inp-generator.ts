...
export function getInpContent(nodes: WhamoNode[], edges: WhamoEdge[]) {
  ...
  allNodeIds.forEach(nodeId => {
    ...
    if (connections.incoming.length > 0 && connections.outgoing.length > 0) {
      ...
      if (inElem.match(/^C\d+$/) && outElem.match(/^C\d+$/)) {
        ...
        if (outNum === inNum + 1 && isTerminalTransition) return;
      }
      ...
      if (isAfterJunction) {
        ...
        if (pattern) {
          if (branchPatterns.has(pattern)) return;
  ...
  edges.filter(e => e.data?.type === 'conduit').forEach(e => {
    ...
    if (!d.variable) addL(` DIAM ${d.diameter}`);
    ...
    if (d.numSegments !== undefined) addL(` NUMSEG ${d.numSegments}`);
    addL('FINISH');
    addL('');
  });
...
export function generateInpFile(nodes: WhamoNode[], edges: WhamoEdge[]) {
  const content = getInpContent(nodes, edges);
